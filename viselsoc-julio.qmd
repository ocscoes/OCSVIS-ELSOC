...
shiny::selectInput(inputId ="subdim_ranking",label = "Selecciona una dimensión",
    choices = sort(unique(df_categ_jerarquica_ranking$subdimension)),
    selected = sort(unique(df_categ_jerarquica_ranking$subdimension))[1]
                   )

shiny::checkboxGroupInput(
  inputId = "ola_ranking",
  label = "Selecciona los años:",
  choices = unique(df_categ_jerarquica_ranking$ola),
  selected = unique(df_categ_jerarquica_ranking$ola)[1:length(unique(df_categ_jerarquica_ranking$ola))],
  inline = TRUE
)
...
# Ranking ---------------------------------------------------------------------
# Data is loades in the setup chunk
load("data/bases-vis-elsoc/df_categ_jerarquica_ranking.RData")
df_long <- df_categ_jerarquica_ranking %>% dplyr::filter(valor=="Alto")
df_long$ola <- factor(df_long$ola,ordered = T) 
df_long$prop <- df_long$prop*100

## Datos reactivos ----
bar_data <- reactive({
  req(input$subdim_ranking, input$ola_ranking)
  ...
  # If ola is a factor, drop unused levels or coerce:
  out <- out %>%
    dplyr::mutate(
      # choose ONE of these:
      # ola = forcats::fct_drop(ola)
      ola = as.character(ola)         # or as.integer(as.character(ola))
    )

  out
}) %>% 
  # Cachea por estos inputs (opcional, mejora performance en apps grandes)
  shiny::bindCache(input$subdim_ranking, input$ola_ranking, input$ika)
...
output$bar_plot <- renderPlotly({
  ...
  validate(
    need(!all(is.na(df_long_for_barplot$prop)), 
  ...
  plot_ly(
    ...
    text = ~paste0(
      "Año: ", ola,
      "<br>Indicador: ", indicador,
      "<br>Proporción: ", round(0, 100), "%"
    )
  ) %>%
    layout(
      barmode = "group",
      title  = paste0("Proporción por año — ", input$subdim_ranking),
  ...
  # Asegura que el render se dispare al cambiar estos inputs
